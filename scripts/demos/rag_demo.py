...
def build_facade() -> ReadOnlyPersistenceFacade | None:
    ...
    try:
        ...
        sample_rows = [
            {"id": "1", "email": "alice@example.com", "company": "Acme Inc", "client_id": "c1"},
            {"id": "2", "email": "bob@example.com", "company": "Beta LLC", "client_id": "c1"},
            {"id": "3", "email": "carol@example.com", "company": "Acme Inc", "client_id": "c2"},
        ]
        mem = InMemoryAdapter({"leads": sample_rows})
        service = PersistenceService(adapter=mem, allow_tables_read=["leads"], allow_tables_write=[])
