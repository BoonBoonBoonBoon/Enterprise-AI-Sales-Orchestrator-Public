...
def main() -> None:
    ...
    envelope = task(
        ...
        source="orchestrator_write_demo",
        destination=rconf.STREAM_TASKS_WRITE,
        priority=Priority.NORMAL
    )
    
    r = RedisPubSub()
    # Enqueue the task
    mid = r.xadd(rconf.STREAM_TASKS_WRITE, to_redis_fields(envelope), maxlen=int(os.getenv("STREAM_MAXLEN", "0") or 0) or None)
    print(json.dumps({
        "enqueued": True, 
        "message_id": mid, 
    ...
    try:
        (Path(__file__).parent / ".last_write_task_id").write_text(envelope.metadata.task_id, encoding="utf-8")
    except Exception:
        pass
    ...
    # Wait for the result on the results stream that matches our correlation_id
    res = r.wait_for_stream(
        ...
        predicate=lambda m: isinstance(m, dict) and (
            ...
            m.get("task_id") == envelope.metadata.task_id  # Fallback for backwards compat
        ),
        timeout=15.0,
        block_ms=1000,
    )
