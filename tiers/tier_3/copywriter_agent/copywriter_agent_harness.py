"""
Copywriter Agent Harness

Wraps Copywriter Agent with:
...
class CopywriterAgentHarness:
    """
    Harness wrapper for Copywriter Agent providing reliability layer.
    ...
    def __init__(
        ...
        """
        Initialize Copywriter Agent Harness.
        
        Args:
            agent: CopywriterAgent instance to wrap
    ...
    async def execute(
        ...
        """
        Execute copywriting task with retry logic and observability.
        
        Args:
            task_data: Task data for copywriting operation
        ...
        _trace_log(
            ...
            step="received",
            detail="copywriting task received",
            request_id=request_id,
        )
        
        logger.info(
            f"[{request_id}] Starting copywriting task",
            extra={
                "request_id": request_id,
                "task_data": task_data,
                "attempt": attempt
        ...
        while attempt <= self.max_retries:
            try:
                ...
                logger.info(
                    f"[{request_id}] Copywriting task completed successfully",
                    extra={
                        "request_id": request_id,
                ...
                _trace_log(
                    ...
                    task_id=task_identifier,
                    step="completed",
                    detail="copywriting task completed",
                    attempts=attempt + 1,
                    execution_time=execution_time,
                )
                ...
                if attempt > self.max_retries:
                    ...
                    logger.error(
                        f"[{request_id}] Copywriting task failed after {attempt} attempts",
                        extra={
                            "request_id": request_id,
                            "error": str(e),
                            "execution_time": execution_time,
                ...
                logger.warning(
                    f"[{request_id}] Copywriting task failed, retrying in {backoff}s",
                    extra={
                        "request_id": request_id,
                        "error": str(e),
                        "attempt": attempt,
                ...
                _trace_log(
                    ...
                    task_id=task_identifier,
                    step="retrying",
                    detail="retrying copywriting task",
                    attempt=attempt,
